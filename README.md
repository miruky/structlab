...
B木タブでは、キーの挿入・削除・検索のたびに、根から葉までの探索経路、満杯ノードの分割、キー不足時の兄弟からの借用とノード統合、根の縮退といった内部の動きが、構造図のハイライトと日本語の操作ログの両方に現れます。最小次数を t = 2 と t = 3 で切り替えると、同じキー列でも木の形がどう変わるかを見比べられます。ハッシュテーブルタブでは、チェイン法と線形走査法を切り替えながら、FNV-1aハッシュ値の計算、衝突、線形走査の墓石、負荷率の上限超過による再ハッシュを観察できます。
...
- ハッシュテーブル: 文字列キーを入力して挿入・削除・検索。方式の変更も挿入順を保って作り直します
- どの操作も、内部で起きたことが操作ログに1ステップずつ記録されます
...
- `src/lib/hashtable.ts` — ハッシュテーブル本体。チェイン法と線形走査法、FNV-1a、墓石、再ハッシュ
- `src/lib/treelayout.ts` — B木のSVG座標計算。子は親のキー境界からぶら下がる
- `src/app.ts` — タブ・コントロール・イベント再生・FLIPアニメーション
